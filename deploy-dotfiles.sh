...
deploy_dwm() {
    ...
    sudo stow -t / xsessions
    sudo stow -t / startdwm
    echo "Done."
}
...
while getopts ":ha:S:s:c:w:d:" o; do case "${o}" in
					 ...
					 ;;
				      c) deploy_config_dotfiles
					 ;;
				      d) deploy_dwm
					 ;;
				      s) test -z ${OPTARG} || SUCKLESS_DIR=${OPTARG}
					  deploy_suckless
					 ;;
				      S) test -z ${OPTARG} || SCRIPTS_DIR=${OPTARG}
					  deploy_scripts
					 ;;
				      w) test -z ${OPTARG} || WALLPAPER_DIR=${OPTARG}
					  deploy_wallpaper
					 ;;
